...
CONFIG = {
    ...
    "nhap_pattern": "*CT.NHAP_processed.csv",
    "xuat_pattern": "*CT.XUAT_processed.csv",
    "output_dir": Path.cwd() / "data" / "product",
...
def aggregate_by_year(group: pd.DataFrame) -> Dict[str, float]:
...
def process_nhap_data() -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # 2. Summary: Mã hàng mới | Mã hàng | Tổng số lượng | Tổng thành tiền | ...
    year_cols = [col for col in product_df.columns if col.startswith("Tổng")]
    summary_cols = ["Mã hàng mới", "Mã hàng"] + sorted(year_cols)
...
def process_xuat_data(product_info_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Reorder columns
    year_cols = [col for col in summary_df.columns if col.startswith("Tổng")]
    summary_cols = ["Mã hàng mới", "Mã hàng"] + sorted(year_cols)
...
def calculate_fifo_cost(
    ...
    nhap_group["Đơn giá"] = nhap_group["Đơn giá"].replace([float('inf'), float('-inf')], 0)
...
def process_inventory_data(
    ...
    for _, row in product_info_df.iterrows():
        ...
        nhap_info = nhap_by_product.get(ma_hang, {
...
def process_nhap_price_data(
    ...
    for ma_hang in product_info_df["Mã hàng"].unique():
        ...
        first_date = product_data.iloc[0]["Ngày"].strftime("%Y-%m-%d") if pd.notna(product_data.iloc[0]["Ngày"]) else ""
        last_date = product_data.iloc[-1]["Ngày"].strftime("%Y-%m-%d") if pd.notna(product_data.iloc[-1]["Ngày"]) else ""
...
def process_xuat_price_data(
    ...
    for ma_hang in sorted(valid_ma_hang):
        ...
        first_date_str = max_price_entry["first_date"].strftime("%Y-%m-%d") if pd.notna(max_price_entry["first_date"]) else ""
        last_date_str = max_price_entry["last_date"].strftime("%Y-%m-%d") if pd.notna(max_price_entry["last_date"]) else ""
